...
with open(dpstrelka, "a") as writeto: 
  ...
  with open(strelka, "r") as fn: # accept only sample fields
   ...
   for i,line in enumerate(fn):
     if i != 0: # start writing to file after first line
      # header=line[:6]
       #header.extend(line[9:]) 
       line = re.split(r'\t|:', line) # split into list at tabs between samples and colunms inbetween fields in each sample
       dp_samples= line[4::10] # start at 4th index for depth   and jump every 10th element for next depth read 
